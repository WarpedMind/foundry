...
2. **If the user doesn't know the specifics yet**, do not fabricate plausible-sounding regulatory language to fill the gap. Write the section honestly as: "Regulatory context not yet researched — do not treat this project as compliant until this section is filled in with verified, specific requirements." A known gap, stated plainly, is safer than confident-sounding filler that nobody will think to double check later.
   ...
   **The same treatment applies when the user answers confidently but vaguely — not just when they say "I don't know."** A sixth fresh-eyes review (Session 18) found a real gap here: this step's safeguard only triggers on an explicit "I don't know," but Step 1 itself sets a real specificity bar (naming "the actual letter number and date," per the CFTC example). An answer like "we follow GDPR" — no article cited, no date, no check of whether GDPR's territorial/material scope actually applies to this project — fails that bar without ever sounding like "I don't know." Apply the same flag-don't-fabricate logic to a vague-but-confident answer as to an explicit unknown: if the answer doesn't include something checkable (a specific article/section, a date, or a stated reason the framework applies to this project), treat it as unverified — write the honest-placeholder text from this step, not the user's vague claim presented as settled fact. State plainly why: "you said GDPR, but I don't have a specific article or an explanation of why it applies here — I'll mark this unverified rather than write it as confirmed, since a wrong compliance claim is worse than an honest gap."
...
One cross-skill option applies here, and it is deliberately not `/qc-review`. Two attempts at a code-facing trigger were written and both failed the same test: **no step in this skill reads a line of source.** Step 1 asks questions, step 2 writes a placeholder, step 3 asks more questions, step 4 writes the section. A trigger phrased around what live code does is one no run of this skill can evaluate, and the `HANDLES_DATA_OR_MONEY` flag on its own fires on brand-new projects where `foundry-repo-hygiene` sequences code *after* scaffolding, so there is nothing yet to review. `foundry-init` already raises that offer once per run from a position that can actually see the security scan's result. Writing a compliance section is not itself the risky event; handling the data is, and this skill never witnesses it. Separately, if this section went into a CLAUDE.md that already carried one, `/foundry-audit` is worth offering, because a replaced section is where a dated decision cross-reference quietly loses its target. The remaining option is `/foundry-audit`, and the placeholder outcome does not suppress it. Writing "not yet researched" says the *rule* could not be pinned down; it says nothing about whether the project moves money or personal data, which is step 3's separate `HANDLES_DATA_OR_MONEY` question. Nor does it spare the document: replacing an existing governance section with the honest placeholder breaks inbound cross-references exactly as replacing it with researched text would, so if anything the audit offer is *more* warranted there, not less. A project that demonstrably handles either in live code while nobody can name the framework governing it is the run where an adversarial pass is worth the most, not the least — so the code-facing offer stands there, and reading the placeholder as "nothing was handled" would silence it at exactly the wrong moment. Since the Session 18 fix above widened the placeholder path to cover confident-but-vague answers too, that misreading would now fire on the common case rather than a rare one.
