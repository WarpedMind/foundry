...
When this run merged into documents that already existed, `/foundry-audit` belongs in that list too — a merge into a CLAUDE.md with real history is exactly the operation that leaves a cross-reference pointing at a heading which moved. Leave it out after a clean from-scratch render: freshly rendered templates have accumulated no references to break, and auditing a file written thirty seconds ago is the filler this convention guards against.
