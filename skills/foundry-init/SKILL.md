...
Two cross-skill options belong in that list, each gated on what this particular run did. A run should surface `/qc-review` when `foundry-security`'s own trigger fired — its step 3 scan turned up a committed secret, or this run extended the pattern set past the shipped baseline for something project-specific. Deliberately *not* "the run took the `HANDLES_SECRETS=true` branch": that holds on every real init of a secrets-handling project, over a baseline that ships already tested, and a condition every run satisfies is not a condition. The same rationale was rejected inside `foundry-security` for the same reason; relocating it up here would not have made it discriminate. A run down the existing-project path at Step 0-E that merged into hand-written docs should surface `/foundry-audit` — retrofitting is a restructure, and restructuring is the trigger that skill names for itself. Those two gates are independent and both can hold at once: Step 2 routes a retrofit through `foundry-security`'s already-committed-secrets check, so a retrofit of a mature repo that handles credentials is the run where both belong in the menu together. Read the word "and" there, never "or" — that combination is the highest-risk run this orchestrator has, not a case to pick one option for. The throwaway/script path earns neither: a single minimal CLAUDE.md handles no credentials and has nothing to cross-reference.

The orchestrator also owns de-duplication, because nothing else can. A run where `foundry-security`'s scan finds something satisfies its trigger, `foundry-hooks`' Hook 2 trigger, and this skill's own at once — skills that cannot see each other, each correctly concluding the same offer is warranted over the same patterns. Surface it once, at the end of the run that installed them, and suppress it in the sub-skill turns within that run. Three appearances of one suggestion is not three times the signal; it is the desensitization the whole conditional design exists to avoid, arriving by a route each individual skill is right about.
