...
**A missing section is not the same as a confirmed "no constraint" — don't treat it as equivalent.** A sixth fresh-eyes review (Session 18) found this check is silently skipped, not just inapplicable, when no governance section exists yet — which happens routinely via `foundry-init`'s Step 0-E retrofit path (the user can add `foundry-stack` alone without ever running `foundry-governance`), or simply because the questionnaire that would have flagged the project as regulated was answered inaccurately or skipped. If no `REGULATORY CONTEXT`/`COMPLIANCE` section exists, don't proceed straight to the generic fit question as if the absence settles it. Ask once, briefly, whether the project has any confidentiality/regulatory constraints Foundry doesn't already know about — e.g. "I don't see a regulatory or compliance section in this project's docs — is there anything confidential or regulated about this project that should affect how STACK.md gets tracked?" This is a cheap, one-line check, not a re-run of `foundry-governance`'s full questionnaire — if the answer is yes, point the user at `foundry-governance` rather than trying to capture governance-level detail here.
...
**Cite the concrete evidence, not just the conclusion.** A sixth fresh-eyes review (Session 18) found this verification step was the one place in Foundry without a concrete, re-runnable check — every mechanical hook in this project (the secrets-guard regex, the SessionStart merge guard, Hook 3's status logic) ended up with an explicit command or fixture to point to, and every SESSIONS.md entry in this repo cites real evidence (a passing test count, a live log line) rather than asserting "verified" on its own say-so. Hold STACK.md entries to the same bar: when marking something "in use," name the actual evidence inline or in the session notes — the test count (`pytest: 49/49 passed`), the live log line that proves it ran, or the command used to confirm it — not just the word "verified." If no such evidence exists yet, that's the signal to mark the entry "pending verification" per the paragraph above, not write "in use" on the strength of the code existing.
...
End the turn with an `AskUserQuestion` carrying concrete options instead of an open invitation. What's usually still open after this skill runs: add a version/milestone snapshot for what just shipped, fill in a "why this over the alternatives" note (or a DECISIONS.md cross-reference) on a row that currently only names a technology, promote a "pending verification" entry now that there's real evidence for it, or stop here. Two to four options; "Other" comes free with `AskUserQuestion`.
