...
**One check measures cost rather than correctness.** The `auto-load budget` line is the size of the documents loaded into every session before you type anything — CLAUDE.md plus whatever the SessionStart hook injects. It reports the total, which file is biggest, and what percentage of the budget you're using, on every run including a clean one. That last part is the point: these files grow a little every session, nothing about them is ever *wrong*, so no other check can see it happening. Foundry's own repo reached ~38% of a context window this way before anyone measured. Read the percentage, not just the PASS.
