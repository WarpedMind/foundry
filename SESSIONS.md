...
### Test harness
Not applicable for either finding this session — both are documentation/orchestration-prose gaps, not mechanical commands or regex patterns. Claude Code's traversal behavior and per-interface hook-rendering behavior are both platform constraints outside Foundry's control; nothing here is a `run_fixtures.sh`-testable case. Recorded explicitly rather than silently skipped, consistent with Session 18's prose-only findings.
