...
- [ ] **`CwdChanged`-based cross-project drift detection hook — designed conceptually, NOT built.** The idea: a hook that fires when the working directory changes mid-session to a different project (different git remote / different top-level directory entirely, not just a subdirectory of the same repo), and surfaces a visible suggestion to checkpoint the prior project's SESSIONS.md and consider `/clear` before continuing — catching exactly the kind of cross-project context drift that motivated this idea (a long conversation that started in one project and drifted into an unrelated one with nothing ever flagging the handoff). **Why this isn't built yet:** `CwdChanged` is a real, valid Claude Code hook event, but its actual stdin payload shape (does it provide old/new cwd explicitly, or just fire with no payload?) could not be verified from within the session that conceived this — unlike `Bash`/`Write` hooks, which can be pipe-tested with synthetic stdin (`echo '{}' | bash -c "<command>"`), `CwdChanged` requires a genuine harness-level directory-change event to fire, which isn't something that could be synthesized in-conversation. Writing a hook against a guessed payload shape would repeat the exact "tested" overclaim mistake from the Session 4 security review. A weaker, written-rule-only version of this same idea (no hook, just an instruction to proactively suggest a checkpoint when the directory has changed to a different project) was added to the CLAUDE.md template's standing Rules in the meantime — verify the real `CwdChanged` payload shape (e.g. via `/hooks` or a real cross-project session) before building the actual hook.
