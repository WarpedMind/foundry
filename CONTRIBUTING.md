...
- **If you touched `skills/foundry-audit/audit.sh`**: add a mutation case to the `foundry-audit` suite in `tests/run_fixtures.sh` before you trust the change. A new check needs a case that injects a real instance of the defect it claims to catch; a tightened check needs a case in the other direction too, proving it still stays quiet on the input it's meant to ignore. The bar for this file is higher than for the others on purpose — every other suite tests a pattern, this one tests whether a *check can fail at all*, which is the property the entire skill's credibility rests on. If a check can't be mutation-tested (it needs semantic judgment rather than pattern matching), it doesn't belong in the script — put it in `SKILL.md` labeled as judgment, where it can't borrow credibility from the tested ones.
...
This re-runs seven adversarial suites: the `.gitignore` baseline and secrets-guard pre-commit regex (case lists in `tests/fixtures/`), Hook 4's directory-drift detection, Hook 3's status/offer logic, Hook 5's push-time `qc-review` offer, the Hook 1/3 SessionStart merge guard, and `foundry-audit`'s mutation tests (the latter five as inline cases in the script itself, since they parse structured input — or in the audit's case need a whole synthetic project tree on disk — rather than matching a static filename against a pattern). The audit suite is shaped differently from the rest and worth understanding before extending it: it builds a deliberately clean fixture project, asserts the audit reports it clean, then injects exactly one known defect per check class and asserts that specific check catches it. It's also run automatically on every push/PR via `.github/workflows/fixtures.yml`. Prose-only skills (`foundry-governance`, `foundry-stack`, `foundry-repo-hygiene`'s sequencing) have no mechanical command to test this way and are still verified by direct invocation against a scratch directory or by re-reading the applied instruction text for internal consistency (see each skill's own `SKILL.md` for its specific verification steps). `tests/calibration/qc-review/` is a separate, deliberately uncovered eighth item — manual, non-deterministic (dispatches a real LLM subagent), and never added here; see its own README.md for its procedure.
