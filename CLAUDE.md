...
## KNOWN DEBT
- `qc-review` has only been tested once, on one project, with one obvious bug class (weak crypto/auth) deliberately planted to be found — still untested: a case where it correctly finds nothing (confirming it reports that plainly rather than padding), and the "general/unclear" fallback focus category for scope outside auth/payments/destructive-actions
...
- Test `qc-review`'s true-negative case (review something genuinely clean) and its general/unclear fallback focus — both untested so far
- Consider cross-referencing `qc-review`'s proactive-offer criteria from `foundry-security`/`foundry-governance`'s own SKILL.md files, not just `foundry-init`'s Step 3
